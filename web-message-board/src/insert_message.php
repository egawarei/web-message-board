...
if ($is_valid_auther_name && $is_valid_message) {
    ...
    // クエリを実行する
    $stmt->execute();
    //投稿者名、投稿内容のバリデーションが通った場合に、投稿完了のテキスト「投稿しました」
    $_SESSION['action_success_text'] = '投稿しました';
    $_SESSION['action_error_text'] = '';
    $_SESSION['input_error_author_name'] = '';//投稿者名のエラーを表示します
    $_SESSION['input_error_message'] = '';//投稿内容のエラーを表示します
    $_SESSION['input_pre_author_name'] = '';//投稿者名の文字列
    $_SESSION['input_pre_message'] = '';//投稿内容の文字列
}else {
    //バリデーションが通らなかった場合に、エラー
